...
import {
  IOrganizerItem,
  IStructureChild,
} from "../../../core/interfaces/organizer.interface";
...
export class EditOrganizerComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe((params) => {
      ...
      if (organizerId) {
        this.organizerService.getOrganizer(organizerId).subscribe((data) => {
  ...
  onSubmit(): void {
    ...
    this.organizerService.updateOrganizer(this.organizer).subscribe({
      next: () => {
        this.toastrService.show("", "Organizador atualizado com sucesso!", {
          status: "success",
          duration: 8000,
        });
        this.router.navigate(["/pages/management"], {
          state: { highlightOrganizerId: this.organizer.id },
        });
      },
    });
  }
